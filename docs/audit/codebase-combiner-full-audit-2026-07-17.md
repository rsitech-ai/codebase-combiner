...
| App Store evidence         | The package lacked a bundled license, operation lock, and source-bound checksums.                                        | The bundle includes MIT `LICENSE`; packaging emits `release-manifest.json` and verified `SHA256SUMS` under a fail-closed lock.     |
| Notarization               | Final checksums selected the first lexicographic SBOM and symbols archive instead of the manifest identities.            | The script validates and resolves exact artifact basenames from `release-manifest.json`.                                           |
